...
def ml_insert():
    ...
    model_lgbm = LGBMRegressor(objective='rmse', n_estimators=3000, learning_rate=0.01)
    eval_set_ALLRESTS = [(X_train, y_train)]
    model_lgbm.fit(X_train, y_train, eval_set=eval_set_ALLRESTS, eval_metric='rmse', early_stopping_rounds=15,
                   verbose=20)
    elm_test["Prediction"] = model_lgbm.predict(X_test)

    pjme_all = pd.concat([elm_test, elm_train], sort=False)


    print(elm_test[elm_test['Prediction'] < 1000])
    # plot
    pjme_all = pd.concat([elm_test, elm_train], sort=False)
    ...
    print('-------')
    pridction = [{"name": "ilens.live_data.raw", "datapoints": tag_1, 'tags': {"category_3": "device_instance_188", "category_5": "tag_10093",
                                                                               "category_1": "industry_3_client_1107", "category_2": "gateway_instance_2"}} ]
