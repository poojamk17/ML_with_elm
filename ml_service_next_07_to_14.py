...
def ml_insert():
    ...
    kairos.set_metrics_tags({"category_3": "device_instance_165", "category_5": "tag_293",
                             "category_1": "industry_3_client_1107", "category_2": "gateway_instance_92"})
    ...
    df2 = lag_features(df2, [672,768,1344])
    ...
    # removing -ve value
    pjme_neg = pjme_test[pjme_test['MW_Prediction'] < 0]

    print('mae   ',mean_absolute_error(y_true=test['Value'],
                   y_pred=pjme_test['MW_Prediction']))

    for i in range(len(pjme_neg)):
        pjme_neg['MW_Prediction'][i] = pjme_train[
            (pjme_train['Day'] == pjme_neg['Day'][i]) & (pjme_train['Hour'] == pjme_neg['Hour'][i]) & (
                        pjme_train['Minute'] == pjme_neg['Minute'][i])].mean()['Value']
    pjme_test.update(pjme_neg)
    ...
    print('-------')
    # pridction = [{"name": "ilens.live_data.raw", "datapoints": tag_1, 'tags': {"category_3": "device_instance_165", "category_5": "tag_10074",
    #                                                                            "category_1": "industry_3_client_1107", "category_2": "gateway_instance_92"}} ]
