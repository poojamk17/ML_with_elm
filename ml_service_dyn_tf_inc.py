...
def ml_insert():
    ...
    df2 = df[(df['Value'] > (np.mean(df['Value']) - (3 * np.std(df['Value'])))) & (
                df['Value'] < (np.mean(df['Value']) + 0.96* np.std(df['Value'])))]
...
#
    ...
    # Data
    mean_per_hour = df2.groupby("Hour")["Value"].agg(["mean"])
...
#
    ...
    # train_test_date = datetime.today().replace(microsecond=00,second=00)
    train_test_date=lastone
...
#
    ...
    # evals_result = xgd_reg.evals_result()
    # print(evals_result)

    consum_test["Prediction"] = xgd_reg.predict(X_test)
    consum_train["Prediction"] = xgd_reg.predict(X_train)
    print('Actually value and predicated value',consum_train[["Value", "Prediction"]])
    ...
    mean_sq_tr = mean_squared_error(y_train, xgd_reg.predict(X_train))
    rmse_tr = np.sqrt(mean_sq_tr)
    mean_abs_sq_tr = mean_absolute_error(y_train, xgd_reg.predict(X_train))
    print("Root Mean Squared Error train: {}".format(rmse_tr))
    print("Mean Absolute Error train : {}".format(mean_abs_sq_tr))


    from sklearn.metrics import r2_score
    print('r2_test---->', r2_score(y_test, xgd_reg.predict(X_test)))
    print('r2_train---->', r2_score(y_train, xgd_reg.predict(X_train)))


    consum_test["Difference"] = np.abs(consum_test["Value"] - consum_test["Prediction"])
    consum_test["Difference"].sort_values(ascending=False)[:500]

    consum_train["Difference"] = np.abs(consum_train["Value"] - consum_train["Prediction"])
    consum_train["Difference"].sort_values(ascending=False)[:500]

    worst_days = consum_test.groupby(['Year', 'Month', 'Dayofmonth']).mean()[['Value', 'Prediction', 'Difference']]
    print('worst days---->',worst_days.sort_values(by="Difference", ascending=False)[:10])

    print('best days----->',worst_days.sort_values(by="Difference", ascending=True)[:10])
    ...
    # # diffe = [{"name": "ilens.live_data.raw", "datapoints": tag_2, 'tags': {'category_5':'tag_10075' , 'category_3':'device_instance_188'}} ]
    # # kairos.update_kairos_data(True, diffe)
    # #
    # # raw = [{"name": "ilens.live_data.raw", "datapoints": tag_3, 'tags': {'category_5':'tag_10076' , 'category_3':'device_instance_188', "category_1": "industry_3_client_1107", "category_2": "gateway_instance_2"}} ]
